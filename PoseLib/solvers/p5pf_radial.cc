...
int p5pf_radial(const std::vector<Eigen::Vector2d> &points2d, const std::vector<Eigen::Vector3d> &points3d,
                std::vector<CameraPose> *output_poses, std::vector<double> *output_focals,
                std::vector<double> *output_dist, bool normalize_input) {
    if (normalize_input) {
        double focal0 = 0.0;
        for (int i = 0; i < 5; ++i) {
        ...
        int n_sols = p5pf_radial(scaled_points2d, points3d, output_poses, output_focals, output_dist, false);
    ...
    for (size_t i = 0; i < poses_radial.size(); ++i) {
        ...
        // (f + f*d*r^2) * (RX(1:2) + t(1:2)) = (RX(3) + t(3)) * x
        // [RX(1:2)+t(1:2), r^2*(RX(1:2)+t(1:2)), -x] * [f; f*d; t3] = RX(3)*x
        CameraPose p = poses_radial[i];
        for (int k = 0; k < 5; ++k) {
            const double r2 = points2d[k].squaredNorm();
            const Eigen::Vector3d RX = p.rotate(points3d[k]);
            const Eigen::Vector2d RX2t = RX.topRows(2) + p.t.topRows(2);
        ...
        const Eigen::Vector3d sol = AtA.inverse() * Atb;
        double focal = sol(0);
        double dist = sol(1);
        p.t(2) = sol(2);
